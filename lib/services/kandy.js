...
function upsertUser(userDescription) {
    return getUserAccessToken(userDescription.username)
        // If it fails, create the user
        .catch(() => createUser(userDescription))
        // Finally return the user description
        .then(() => userDescription);
}
